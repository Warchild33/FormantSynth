...
const int LFO_RATE = 441;
const int LFO_SAMPLE_PERIOD = floor(48000 / LFO_RATE);
double LFO_FREQUENCY_TABLE[] = { // see https://github.com/smbolton/hexter/tree/master/src/dx7_voice.c#L1002
...
double LFO_AMP_MOD_TABLE[] = { // TODO: use lfo amp mod table
...
double LFO_PITCH_MOD_TABLE[] = {
    0, 0.0264, 0.0534, 0.0889, 0.1612, 0.2769, 0.4967, 1
};

const int LFO_MODE_TRIANGLE = 0;
const int LFO_MODE_SAW_DOWN = 1;
const int LFO_MODE_SAW_UP = 2;
const int LFO_MODE_SQUARE = 3;
const int LFO_MODE_SINE = 4;
const int LFO_MODE_SAMPLE_HOLD = 5;

const int LFO_DELAY_ONSET = 0;
const int LFO_DELAY_RAMP = 1;
const int LFO_DELAY_COMPLETE = 2;

// Private static variables
double phaseStep = 0;
double pitchModDepth = 0;
double ampModDepth = 0;
double sampleHoldRandom = 0;
double delayTimes[] = {0, 0, 0};
double delayIncrements[] = {0, 0, 0};
int delayVals[] = {0, 0, 1};
LfoParams params;
...
double Lfo::render() {
    ...
    if (this->counter % LFO_SAMPLE_PERIOD == 0) {
        ...
        amp *= this->delayVal;
        pitchModDepth = 1 +
            LFO_PITCH_MOD_TABLE[params.lfoPitchModSens] * (params.controllerModVal + (double)params.lfoPitchModDepth / 99.);
        ...
        if (this->phase >= PERIOD) {
            sampleHoldRandom = 1 - (double)rand()/RAND_MAX * 2;
            this->phase -= PERIOD;
        }
    }
    this->counter++;
...
void Lfo::update()
{
    double frequency = LFO_FREQUENCY_TABLE[lfoSpeed];
    phaseStep = PERIOD * frequency/LFO_RATE; // radians per sample
    ampModDepth = (double)params.lfoAmpModDepth * 0.01;
    // ignoring amp mod table for now. it seems shallow LFO_AMP_MOD_TABLE[params.lfoAmpModDepth];
    delayTimes[LFO_DELAY_ONSET] = (LFO_RATE * 0.001753 * pow(params.lfoDelay, 3.10454) + 169.344 - 168) / 1000;
    delayTimes[LFO_DELAY_RAMP] = (LFO_RATE * 0.321877 * pow(params.lfoDelay, 2.01163) + 494.201 - 168) / 1000;
